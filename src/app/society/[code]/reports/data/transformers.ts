import type { RawSocietyReportData } from "./queries"
import type {
  SocietySummary,
  BillCategoryItem,
  ExpenseCategoryItem,
...
export function transformSocietyReportData(
  society: SocietyReportData["society"],
  rawData: RawSocietyReportData
): SocietyReportData {
  const {
    billAggregate,
    paymentAggregate,
  ...
  const billsByCategory: BillCategoryItem[] = billsByTypeRaw.map((b) => {
  ...
  const expensesByCategory: ExpenseCategoryItem[] = Array.from(expenseMap.entries())
  ...
  // Payments by Mode
  const paymentsByMode: PaymentModeItem[] = paymentsByModeRaw.map((p) => {
  ...
  const unitLedger: UnitLedgerItem[] = []
  const votingList: StatutoryData["votingList"] = []
  ...
  for (const flat of flats) {
    ...
    let accountStatus: UnitLedgerItem["accountStatus"] = "CLEAR"
    ...
    if (flatOutstanding > 0) {
      let agingBucket: DefaulterItem["agingBucket"] = "DAYS_0_30"
  ...
  const vendorAging: VendorAgingItem[] = Array.from(vendorMap.values()).map(
    (v) => {
      let agingBucket: VendorAgingItem["agingBucket"] = "DAYS_0_30"
  ...
  for (const b of budgetsRaw) {
    for (const item of b.items) {
      ...
      let status: BudgetVarianceItem["status"] = "ON_TRACK"
  ...
  const monthlyTrends: MonthlyTrendItem[] = Array.from(monthlyMap.entries()).map(([key, item]) => {
    const rate =
      item.billedAmount > 0
  ...
  // One-time campaigns
  const oneTimeCampaigns: OneTimeFundsData["campaigns"] = oneTimeCollectionsRaw.map(
    (c) => {
  ...
  const memberDeposits: OneTimeFundsData["deposits"] = memberDepositsDetailed.map(
    (d) => ({
      id: d.id,
  ...
  const oneTimeFunds: OneTimeFundsData = {
  ...
  // Bank accounts, FDs, Fixed Assets
  const bankAccounts: BankAccountItem[] = accounts.map((a) => ({
    id: a.id,
    name: a.name,
    bankName: a.bankName,
  ...
  const fixedDepositsList: FixedDepositItem[] = fixedDeposits.map((fd) => ({
    id: fd.id,
  ...
  const fixedAssetsList: FixedAssetItem[] = fixedAssetsRaw.map((a) => ({
    id: a.id,
    name: a.name,
  ...
  // Cheques
  const cheques: ChequeItem[] = chequesRaw.map((c) => ({
    id: c.id,
  ...
  const financialYears: FinancialYearItem[] = financialYearsRaw.map((fy) => ({
    id: fy.id,
    name: fy.name,
  ...
  return {
    ...
    paymentsByMode,
    bankAccounts,
    fixedDeposits: fixedDepositsList,
