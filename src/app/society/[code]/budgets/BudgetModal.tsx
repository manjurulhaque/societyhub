...
export function BudgetModal({
  ...
  // Pre-populate on open or edit change
  useEffect(() => {
    if (budget) {
      setName(budget.name)
      setFinancialYearId(budget.financialYearId)
      setItems(
        budget.items.map((i) => ({
          id: i.id,
          ledgerId: i.ledgerId,
      ...
      setName("")
      // Default to current active unlocked FY
      const current = financialYears.find((fy) => fy.isCurrent && !fy.isLocked)
      const fallback = financialYears.find((fy) => !fy.isLocked)
      setFinancialYearId(current ? current.id : fallback ? fallback.id : "")
      
      // Default 1 empty row with first ledger
      setItems(
        ledgers.length > 0
    ...
    setError(null)
  }, [budget, financialYears, ledgers, isOpen])
